...
def save_features(feat, file_name):
...
def all_spec(wav, hparams):
    D = _stft(preemphasis(wav, hparams.preemphasis, hparams.preemphasize), hparams)
    mel = _amp_to_db(_linear_to_mel(np.abs(D), hparams), hparams) - hparams.ref_level_db
    linear = _amp_to_db(np.abs(D), hparams) - hparams.ref_level_db
    
    
    if hparams.signal_normalization:
        mel=_normalize(mel, hparams)
        linear= _normalize(linear, hparams)
    return D, mel, linear
